...
class FM_GAN(BaseModel):
    def __init__(self, gpu_id, learning_rate, loss_type, input_dim, z_dim, ae_h_dim_list, dis_h_dim_list):
        super(FM_GAN, self).__init__(gpu_id, learning_rate, loss_type, input_dim, z_dim) 
    ...
    def build_model(self):
        with tf.device('/gpu:%d' % self.gpu_id):
            ...
            ### Discriminating ###
            dis_logit_real, dis_last_hidden_real = self.discriminator_with_last_hidden(self.X, self.dis_h_dim_list, 1, self.keep_prob, False)
            dis_logit_fake, dis_last_hidden_fake = self.discriminator_with_last_hidden(self.gen_X, self.dis_h_dim_list, 1, self.keep_prob, True)
            ...
            ### Loss ###
            self.dec_loss = tf.losses.mean_squared_error(dis_last_hidden_real, dis_last_hidden_fake)
