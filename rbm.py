...
class RBM(BaseModel):
    def __init__(self, gpu_id, learning_rate, loss_type, input_dim, z_dim, ae_h_dim_list):
        super(RBM, self).__init__(gpu_id, learning_rate, loss_type, input_dim, z_dim)
    ...
    def build_model(self):
        with tf.device('/gpu:%d' % self.gpu_id):
            ...
            self.w = tf.get_variable(name='weight', shape=[self.input_dim, self.z_dim], initializer=self.w_init)
            self.hb = tf.get_variable(name='weight', shape=[self.input_dim, self.z_dim], initializer=self.w_init)
            self.vb = tf.get_variable(name='weight', shape=[self.input_dim, self.z_dim], initializer=self.w_init)
            """
            ### Encoding ###
